...
def process(source_key, source, destination, resources, please_stop=None):
    ...
    for i, line in lines:
        ...
        try:
            ...
            task = http.get_json(expand_template(STATUS_URL, {"task_id": task_id}), retry=RETRY, session=session)
            if task.code == u'ResourceNotFound':
                Log.warning("Can not find task {{task}} while processing key {{key}}", key=source_key, task=task_id)
                continue
            ...
            for a in artifacts:
                ...
                # elif a.name.endswith("/buildbot_properties.json"):  # GENERATED BY UPLOADER, NOT THE ACTUAL JOB
                #     bb = read_buildbot_properties(source_key, a.url)
                #     normalized = set_default(bb, normalized)
...
buildbot_translator = BuildbotTranslator()
def read_buildbot_properties(source_key, url):
    try:
        response = http.get(url)
        bb = convert.json2value(convert.utf82unicode(response.content))
        return buildbot_translator.parse(bb)
    except Exception, e:
        Log.warning("Could not translate buildbot while processing key {{key}}\n{{data}}", key=source_key, data=bb)
...
def _normalize(source_key, task_id, tc_message, task, resources):
    ...
    try:
        command = [cc for c in consume(task, "payload.command") for cc in listwrap(c)]   # SOMETIMES A LIST OF LISTS
    ...
    remaining_keys = set([k for k, v in task.leaves()] + [k for k, v in tc_message.leaves()])
    if remaining_keys:
...
def get_tags(source_key, task, parent=None):
    ...
    g = [(k, consume(task.payload, k)) for k in ["supersederUrl", "onExitStatus"]]
