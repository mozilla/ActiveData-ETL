...
The problem with *Data* objects is they have an useless distinction between attributes and properties.  This prevents us from using the `[]` operator for dereferencing; forcing us to use the verbose `__getattr__()` instead.  It also prevents the use of query operators over these objects.
